...
var categoryList = ['id', 'firstName', 'lastName', 'email', 'phone'],
    // category list in the table
currentPage = 1,
    // current (active) page
recordsAmount = 10,
    // amount of data table row on the page
contentList,
    // data array that is used at data operations, generated from 
showList,
    // data array that is shown on the page
...
function buildTable(arr, page) {
  ...
  // fill showList
  for (var i = (currentPage - 1) * recordsAmount; i < currentPage * recordsAmount && i < arr.length; i++) {
    if (!arr[i]) break;
    showList[i] = arr[i];
  }
  ...
  $(categoryList).each(function (index, elem) {
    newItem = "<td class='person-data__table-cell person-data__table-cell_head'>" + elem + "</td>";
    inHTML += newItem;
  });
...
function search() {
  ...
  var filteredData = taskData.map(function (x) {
    return [x.id, x.firstName, x.lastName, x.email, x.phone].join(' ');
  }).reduce(function (sum, val, i) {
